...
class VibesEventHandler implements EventHandler {
  ...
  matchedCount = 0
  totalPostsCounter = 0
  settings = require(this.SETTINGS_PATH).vibesAlgo
  ...
  async updateSettings() {
    ...
    this.settings = require(this.SETTINGS_PATH).vibesAlgo
    this.keywords = this.settings.keywords.map((keyword: string) =>
      keyword.toLowerCase(),
    )
    this.partialKeywords = this.settings.partialKeywords.map(
      (keyword: string) => keyword.toLowerCase(),
    )
    this.negativeKeywords = this.settings.negativeKeywords.map(
      (keyword: string) => keyword.toLowerCase(),
    )
